...
class BorrowRejectedAdapter: RecyclerView.Adapter<BorrowRejectedAdapter.BorrowRejectedViewHolder>() {
    private lateinit var list: ArrayList<BorrowReturnBook>

    inner class BorrowRejectedViewHolder(val binding: ItemBorrowRejectedBinding): RecyclerView.ViewHolder(binding.root){}

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BorrowRejectedViewHolder {
        return BorrowRejectedViewHolder(ItemBorrowRejectedBinding.inflate(LayoutInflater.from(parent.context), parent, false))
    }
